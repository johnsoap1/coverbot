...
async def send_to_storage(message):
    """Send a single media item to storage group without any caption or sender info"""
    ...
    try:
        ...
        logging.error(f"Storage send failed for msg {message.id}: {e}")
...
async def auto_send_album(user_id, chat_id):
    ...
    # Send each item to storage without caption
    if Config.STORAGE_GROUP_ID:
        logging.info(f"auto_send_album: Sending {count} items to storage (no captions)")
        storage_tasks = [send_to_storage(m) for m in medias]
        await asyncio.gather(*storage_tasks, return_exceptions=True)
 
    # Build media list for album send to user
    all_media = []
...
async def send_single_silent(user_id, chat_id, media):
    ...
    # Send to storage without caption
    await send_to_storage(media)
